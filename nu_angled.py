...
def find_meta_for_simplexes(simplexes, L):
...
def nu_angled_algorithm(f, lb, ub, L, error, max_iters):
    f = count_calls(f)
    simplexes = triangulate(lb, ub)
    points = find_objective_values_for_vertexes(simplexes, f)
    simplexes = find_meta_for_simplexes(simplexes, L)
    pareto_front = get_pareto_front(points)

    tolerance = min([s[-1]['tolerance'] for s in simplexes])
    # previous_approx_min = None
    i = 0
    while tolerance > error and i < max_iters:
    # for i in xrange(max_iters):
        ...
        # if previous_approx_min is None:
        #     previous_approx_min = simplex_to_divide[-1]['approx_min_ABC']
        # else:
        #     tolerance = min([s[-1]['tolerance'] for s in simplexes])
        #     if round(previous_approx_min, 6) > round(tolerance, 6):
        #         # print 'Tolerance test failed:  %d. %f < %f' % (i, previous_approx_min, simplex_to_divide[-1]['approx_min_ABC'])
        #     previous_approx_min = simplex_to_divide[-1]['approx_min_ABC']
        ...
        new_simplex1.append({'mins_AB': find_mins_AB(new_simplex1, L),
        ...
        new_simplex2.append({'mins_AB': find_mins_AB(new_simplex2, L),
...
if __name__ == '__main__':
    ...
    pareto_front, simplexes, f_calls = nu_angled_algorithm(f, lb, ub, L, error, max_iters)
